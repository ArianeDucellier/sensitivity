...
!
                               ,CGIDPSX,CGIDPSZ,CGIDSVX,CGIDSVZ,CGTYPIN
!
IMPLICIT NONE
!
...
!
   DAMP     = 0.0D0
   CALL       CMPDMP(W,DGINVAL(IGNLAYE-1),DGINVBE(IGNLAYE-1),DGINVGA(IGNLAYE-1),DGINVVS(IGNLAYE-1),DAMP,IGIDDMP)
...
!-------------------------------------------------------------------------------------------------------------------------------------------
!
   ZSUP_HS(IFR) = CMPLX(1.0D0,0.0D0)
   ZPUP_HS(IFR) = CMPLX(0.0D0,0.0D0)
!
!-------------------------------------------------------------------------------------------------------------------------------------------
...
!
   IF( ZGG(IFR)                            .EQ.CMPLX(0.0D0,0.0D0)) STOP "ERROR IN SVWAVE: ZGG=0"
   IF((ZGG(IFR)*ZGH(IFR)-ZGF(IFR)*ZGI(IFR)).EQ.CMPLX(0.0D0,0.0D0)) STOP "ERROR IN SVWAVE: ZGG*ZGH-ZGF*ZGI=0"
!
!-------------------------------------------------------------------------------------------------------------------------------------------
...
!
   ...
   ELSE
      ZPUP_OBJ(IFR) = CMPLX(0.0D0,0.0D0)
   ENDIF
!
!-------------------------------------------------------------------------------------------------------------------------------------------
...
!
   ...
   ELSE
      ZPDOWN_OBJ(IFR) = CMPLX(0.0D0,0.0D0)
   ENDIF
!
!-------------------------------------------------------------------------------------------------------------------------------------------
...
IF ( (CGTYPIN.EQ."SPR") .AND. (CGIDSVX.NE."SVX") .AND. (CGIDPSX.NE."PSX") ) THEN
   DGRTSVX(:) = 0.0D0
ELSE
...
IF (OPT_INC.GT.1.0D-4) THEN
   IF ( (CGTYPIN.EQ."SPR") .AND. (CGIDSVZ.NE."SVZ") .AND. (CGIDPSZ.NE."PSZ") ) THEN
      DGRTSVZ(:) = 0.0D0
   ELSE
